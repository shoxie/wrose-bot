...
module.exports = {
  ...
  async run(client, message, args) {
    ...
    //   x => x.name === "DJ" || "dj"
    // );
    // if (!message.member.roles.cache.has(message.guild.roles.cache.find(x => x.name === "DJ" || "dj").id))
    // if (!requiredRole) {
    ...
    let serverQueue = client.queue.get(message.guild.id);
    if (!message.guild.roles.cache.has(x => x.name === "dj" || "DJ")) {
      return message.channel.send({
        embed: {
          color: 15158332,
          title:
    ...
    if(message.member.roles.cache.has(x => x.name === 'dj' || 'DJ')) {
