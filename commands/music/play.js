...
module.exports = {
  ...
  async run(message, args) {
    ...
    async function play() {
      if (!musicModel.voiceChannel) musicModel.voiceChannel = message.member.voiceChannel;
      musicModel.connection = await musicModel.voiceChannel.join();
      const dispatcher = musicModel.connection.playStream(ytdl(musicModel.queue[0], {
        filter: "audioonly",
